...
If during encoding a value greater than 32 bits is discovered, this controls
how the value is handled.  Either a big integer class may be specified or 
twos/hex may be used.  The default is to use the class C<Math::BigInt>.  If an
integer is passed in hex format (i.e. starting with 0x), but the twos option
is not specified, the value will be converted to a 2s complement integer per
...
If the value is not already a big integer object, then the value will first
be converted into a big integer object. This option controls the big
integer class into which the objects will be blessed.  Any bigint class which
properly overloads common math operators should function.
The default is C<Math::BigInt>.

=item twos

When encoding an integer, if the value begins with '0x', it is assumed the value
is a valid 2s complement integer in hexadecimal. If the value is signed (i.e. '-0x'),
an error will be generated.

=item hex

If the value begins with '0x' or '-0x', it is converted to a valid 2s complement
integer and encoded.
...
If during decoding any big integers are discovered (integers greater than
32 bits), this option controls how they will be decoded.  Either the name
of a big integer class may be specified or twos/hex.
The default is class C<Math::BigInt>.

=over 4

=item I<BigInt::Class>

The big integer class which large integers will be blessed into.  Any big integer
class that overloads multiplication and addition should be compatible. The default
is C<Math::BigInt>.

=item twos

Decode large integers directly into a hex string with a '0x' prefix as a 2s
complement integer. This is a good option if blessed objects in the decoded
perl structure are undesirable. Because no conversions are needed, it is
also faster.

=item hex

Decode large integers directly into a signed hex string with a '0x' or '-0x' prefix.
Though not as fast as twos, this is a fast option directly compatible with
Math::BigInt C<from_hex> and C<as_hex> methods.
...
=head2 Hex Conversion Functions

=over 4

=item hex2twos

Converts a signed hex string to a 2s complement hex string.

=item twos2hex

Converts a 2s complement hex string to a signed hex string.
...
For example, an unnamed SEQUENCE nested inside another SEQUENCE causes its children
to be merged into the parent's namespace rather than placed in a sub-hash:

  # ASN.1 - unnamed inner SEQUENCE
  SEQUENCE {
    id   INTEGER,
    SEQUENCE {
      name OCTET STRING,
  ...
  # Perl - all elements share the same namespace
  { id => 42, name => "test", flag => 1 }

  # encode:
  $asn->encode(id => 42, name => "test", flag => 1)

  # decode returns:
  { id => 42, name => "test", flag => 1 }

This is in contrast to a I<named> inner SEQUENCE, where the inner elements are
grouped under a sub-hash keyed by the sequence's name:

  # ASN.1 - named inner sequence
  SEQUENCE {
    id  INTEGER,
    sub SEQUENCE {
      name OCTET STRING,
  ...
  # Perl - inner elements in a sub-hash
  { id => 42, sub => { name => "test", flag => 1 } }

The same namespace-sharing behaviour applies to unnamed SET:

  # ASN.1 - unnamed inner SET
  SEQUENCE {
    id  INTEGER,
    SET {
      name OCTET STRING,
...
And to unnamed CHOICE, where the selected alternative is placed directly in the
parent namespace rather than inside a sub-hash keyed by the CHOICE's name:

  # ASN.1 - unnamed CHOICE
  SEQUENCE {
    id     INTEGER,
    CHOICE {
      label  OCTET STRING,
  ...
  # Perl - the chosen alternative appears in the parent namespace
  { id => 1, label => "foo" }    # when OCTET STRING alternative is chosen
  { id => 1, active => 1 }       # when BOOLEAN alternative is chosen
